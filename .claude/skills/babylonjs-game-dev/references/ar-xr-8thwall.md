...
- **Se você assina `orientationchange`/`resize` para reenquadrar câmera ou HUD, pule o trabalho enquanto a sessão de AR está ativa — mas com UMA exceção, e ela não é opcional.** O engine já detecta mudança de canvas sozinho (compara as dimensões a cada frame no `pre-render` e dispara `onCanvasSizeChange`), então chamar `engine.resize()` por fora é redundante para o *tracking* e é suspeito de deixar a projeção fora de sincronia com o canvas. A exceção: **`engine.resize()` é o único gatilho de `engine.onResizeObservable`, e é nele que o `AdvancedDynamicTexture` de tela cheia recalcula o próprio tamanho.** Pulando sempre, a textura do GUI congela no tamanho antigo enquanto o canvas muda, e o picking passa a mapear o toque para o lugar errado (`x * textureSize.width / engine.getRenderWidth()` em `advancedDynamicTexture.js`): **os controles aparecem e não respondem**. Regra prática: pule o resize por padrão, mas redimensione quando `canvas.clientWidth/Height` divergir de `getRenderWidth/Height() * getHardwareScalingLevel()` além de ~2 px. Em regime normal isso nunca dispara; quando dispara, a alternativa é um ponteiro permanentemente quebrado.
  - O caminho que mais produz essa divergência é **entrar ou sair de tela cheia com a sessão no ar** (a barra de endereço do navegador mobile some/volta e muda a altura CSS do canvas). Teste decisivo, e ele separa os dois casos de forma limpa: entre em AR **já** em tela cheia (o resize acontece antes da sessão subir, quando ainda é permitido) e confirme que o HUD responde; depois **saia** da tela cheia no meio da sessão. Se os controles pararem de responder só no segundo caso, é este bug.
